...
// mer
const canvas = document.getElementById("mer");
const ctx = canvas.getContext("2d");
canvas.width = window.innerWidth;
canvas.height = 200;
...
function dessineVagues() {
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  ctx.beginPath();
  for (let x = 0; x < canvas.width; x++) {
    let y = 20 * Math.sin((x + t) * 0.02) + 50;
    ctx.lineTo(x, y);
  }
  ...
  ctx.fill();
  t += 1;
  requestAnimationFrame(dessineVagues);
}
dessineVagues();
...
function envoyerAvis(event) {
    ...
    if (!text) return;
    const avis = { pseudo: pseudo, commentaire: text };
    let avisListe = JSON.parse(localStorage.getItem("avis")) || [];
    avisListe.push(avis);
    localStorage.setItem("avis", JSON.stringify(avisListe)); //Enregistrer l'avis
    afficherUnAvis(avis);
...
function afficherUnAvis(avis) {
    ...
    newReview.classList.add("avis-bulle");  
    newReview.innerHTML = `<strong>${avis.pseudo}</strong><p>${avis.commentaire}</p>`;
    document.getElementById("avisListe").appendChild(newReview);
}

function afficherTousLesAvis() {
    let avisListe = JSON.parse(localStorage.getItem("avis")) || [];
    avisListe.forEach(avis => afficherUnAvis(avis));
}
window.onload = afficherTousLesAvis;
